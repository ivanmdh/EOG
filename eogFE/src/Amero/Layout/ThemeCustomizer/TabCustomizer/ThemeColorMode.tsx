...
import { ThemeColorModeData } from "@/Data/Layout/ThemeCustomizer";
      ...
      <div className="color-body d-flex align-items-center justify-space-between">
        {ThemeColorModeData.map((data, index) => (
          <div className="color-img mx-1" key={index}>
            <Image width={100} height={74} className="img-fluid" src={`${ImagePath}/${data.image}`} alt="" />
            <div className="customizer-overlay"></div>
            <Button color="" className={`color-btn ${data.buttonClass}`} onClick={() => handleMixLayout(data.class)}>
