    ...
    <div className="customizer-color mb-3 p-2 rounded-3 b-t-primary border-3">
      ...
      <ul className="layout-grid">
        {UnlimitedColorOptions.map((item, index) => (
          <li key={index} className="color-layout" data-attr={`color-${item.name}`} data-primary={item.primary} data-secondary={item.secondary} onClick={() => handleColorChange(item.primary, item.secondary)}>
            <div />
          </li>
        ))}
      </ul>
